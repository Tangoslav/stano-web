...
function displayOffers(data) {
    ...
    offerList.forEach(offer => {
        ...
        if (offer.tags && offer.tags.length > 0) {
            ...
            tags.textContent = `Tags: ${offer.tags.join(', ')}`;
        ...
        if (offer.highlightedProducts && offer.highlightedProducts.length > 0) {
            ...
            const hpTitle = document.createElement('h4');
            hpTitle.textContent = 'Highlighted Products';
            highlightedProductsDiv.appendChild(hpTitle);

            const ul = document.createElement('ul');
            offer.highlightedProducts.forEach(product => {
                const li = document.createElement('li');
                li.textContent = `Product Code: ${product.productCode}`;
                if (product.price) {
                    li.textContent += ` - Price: ${product.price.priceWithVAT} ${product.price.currencyCode}`;
                }
                ul.appendChild(li);
            });
            highlightedProductsDiv.appendChild(ul);
